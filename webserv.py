...
def get_auth_token():
    ...
    response = requests.post('https://id.twitch.tv/oauth2/token',
                             headers={"Content-Type": "application/x-www-form-urlencoded"}, data=query)
...
def twitch_delete_subscriptions():
    ...
    r = requests.get('https://api.twitch.tv/helix/eventsub/subscriptions', headers={
    ...
    print(r)
    for i in r['data']:
        requests.delete(f'{config.TWITCH_BASE_SUB_URL}?id={i["id"]}', headers={
            'Authorization': f'Bearer {token}',
            'Client-ID': config.TWITCH_CLIENT_ID
        })
...
def verify_twitch_message(twitch_message_id, twitch_message_timestamp, twitch_request_body, twitch_sent_signature):
    HMAC_PREFIX = 'sha256='
    hmac_secret = config.TWITCH_SUB_SECRET
    hmac_message = f'{twitch_message_id}{twitch_message_timestamp}{twitch_request_body}'

    signature = HMAC_PREFIX + hmac.new(bytes(hmac_secret, 'ascii'), bytes(hmac_message, 'ascii'),
...
def send_mess():
    broadcaster = next((brd for brd in config.TWITCH_BROADCASTERS if brd['id'] == '42078350'), None)
    for chat in config.NOTIFICATION_CHAT_LIST:
        data = {
            "chat_id": chat,
            "photo": broadcaster['notification_photo'],
...
def handle_event():
    ...
    tw_request_body = request.get_data(True, True, False)  # To get RAW request body data
